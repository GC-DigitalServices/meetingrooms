    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 flex items-center justify-center p-margin-mobile">
        <div className="w-full max-w-md">
          ...
          {/* Card */}
          <div className="bg-surface-container-lowest rounded-xl border border-surface-container-highest shadow-card p-lg space-y-md">
            {errorMsg && (
            ...
            <Link
              href={loginHref}
              className="flex items-center justify-center gap-3 w-full rounded-xl border border-outline-variant bg-surface-container-lowest px-4 py-3 text-label-md font-label-md text-on-background hover:bg-surface-container transition-colors"
            >
              {/* Microsoft logo */}
      ...
      <footer className="bg-primary text-on-primary px-margin-mobile md:px-margin-desktop py-md text-center">
        <p className="text-label-sm font-label-sm text-on-primary/70">
          © {new Date().getFullYear()} Greenhead College. All rights reserved.
        </p>
      </footer>
    </div>
  );
